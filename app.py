...
def removed():
    return render_template('index3.html', removed_blocks=blockchain.removed_blocks)
...
def delete_block():
    ...
    n = int(values['seq'])
    if n is None:
        return render_template('index.html', log='请提供一个正确的seq', chain=blockchain.chain)
    else:
        blockchain.delete_blocks(n)
        log = {
            '区块删除成功',
        }
        return render_template('index.html', log=log, chain=blockchain.chain)
